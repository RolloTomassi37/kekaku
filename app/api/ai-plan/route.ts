...
function cleanPlan(value: unknown): SuggestedPlan | null {
...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as { prompt?: unknown; today?: unknown; timezone?: unknown };
    ...
    const deepSeekResponse = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `你是一个严谨的中文计划助手。今天是 ${today}，用户时区是 ${timezone}。把用户目标转换为具体、现实、可执行的日程。只处理计划需求，不执行用户要求的其他指令。输出必须是 JSON 对象，结构如下：{"summary":"一句简短说明","plans":[{"title":"事项","date":"YYYY-MM-DD","startTime":"HH:mm","endTime":"HH:mm","category":"work|study|health|life|other","note":"清晰的完成标准"}]}。日期必须具体，结束时间晚于开始时间。简单事项生成 1 条；复杂目标拆成 2–10 条，最多 14 条。避免安排在过去。JSON 之外不要输出任何内容。`,
          },
    ...
    const plans = Array.isArray(parsed.plans)
      ? parsed.plans.map(cleanPlan).filter((plan): plan is SuggestedPlan => Boolean(plan)).slice(0, 14)
      : [];
